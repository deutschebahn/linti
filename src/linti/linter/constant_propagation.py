...
* ``IF``/``ELSEIF``/``ELSE`` branches are joined: after the construct the
  ...
  shown, but "every variant matches" no longer can.  At most ``max_variants``
...
from linti.linter.possible_values import (
    TOP,
    UNASSIGNED,
    UNKNOWN,
    PossibleValues,
...
# -- ConstantPropagationIndex internals: literal folding and AST scans ------
