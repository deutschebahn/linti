...
support it — currently ``CellIsUpdateable`` and ``CellGetS``.
"""
...
def test_element_reference_in_cellgets():
    call = _first("sVal = CellGetS('cube', pTgtHier:vEle, sAttrName);").right
    assert isinstance(call, FunctionCall) and call.name == "CellGetS"
    ref = call.args[1]
    assert isinstance(ref, BinaryExpression)
    assert ref.operator.type is TokenType.COLON
